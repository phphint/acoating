...
const SemiAutoGalleryPage: React.FC = () => {
  const [images, setImages] = useState<{ id: string, url: string, title: string, description: string }[]>([]);
  const [categoryName, setCategoryName] = useState<string>('Semi Auto'); // Default category name

  useEffect(() => {
    fetch(
      'https://strapi.acoating.com/api/galleries?filters[Category][$eq]=Semi%20Auto&populate=Image&pagination[start]=0&pagination[limit]=1000'
    )
      ...
      .then((data: ApiResponse) => {
        const loadedImages = data.data.map((item: GalleryItem) => {
          ...
          if (item.attributes.Category && categoryName === 'Semi Auto') {
...
export default SemiAutoGalleryPage;
