...
const EngravingGalleryPage: React.FC = () => {
  const [images, setImages] = useState<{ id: string, url: string, title: string, description: string }[]>([]);
  const [categoryName, setCategoryName] = useState<string>('Engraving'); // Set default category name

  useEffect(() => {
    fetch(
      'https://strapi.acoating.com/api/galleries?filters[Category][$eq]=Engraving&populate=Image&pagination[start]=0&pagination[limit]=1000'
    )
...
export default EngravingGalleryPage;
