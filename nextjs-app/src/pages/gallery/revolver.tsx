...
const RevolverGalleryPage: React.FC = () => {
  const [images, setImages] = useState<{ id: string, url: string, title: string, description: string }[]>([]);
  const [categoryName, setCategoryName] = useState<string>('Revolver'); // Default category name

  useEffect(() => {
    fetch(
      'https://strapi.acoating.com/api/galleries?filters[Category][$eq]=Revolver&populate=Image&pagination[start]=0&pagination[limit]=1000'
    )
      ...
      .then((data: ApiResponse) => {
        const loadedImages = data.data.map((item: GalleryItem) => {
          ...
          if (item.attributes.Category && categoryName === 'Revolver') {
...
export default RevolverGalleryPage;
